...
from lerobot.datasets.dataset_tools import _copy_and_reindex_videos, _copy_and_reindex_data, _copy_and_reindex_episodes_metadata
...
class DebugLeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        repo_id: str,
    ...
    def _check_cached_episodes_sufficient(self) -> bool:
        ...
        if self.hf_dataset is None or len(self.hf_dataset) == 0:
            print("Cached hf_dataset is empty or not loaded.")
            return False
        ...
        if not requested_episodes.issubset(available_episodes):
            print(f"Cached episodes {available_episodes} do not cover requested episodes {requested_episodes}.")
            return False
            ...
            for ep_idx in requested_episodes:
                for vid_key in self.meta.video_keys:
                    ...
                    if not video_path.exists():
                        print(f"Required video file {video_path} for episode {ep_idx} and key {vid_key} is missing.")
                        return False
...
def renumber_episodes(
    dataset: DebugLeRobotDataset,
    output_dir: str | Path | None = None,
) -> LeRobotDataset:
    """Renumber episodes with indices incrementing by 1 and starting from 0

        dataset: The source LeRobotDataset to fix.
        output_dir: Base directory for fixed dataset.
    """
    episode_indices = _get_episode_indices(dataset.meta.episodes)
    if not episode_indices:
        raise ValueError("No episode_index values found in metadata.")
...
def _get_episode_indices(episodes) -> list[int]:
    if episodes is None:
        return []
    if hasattr(episodes, "column_names") and "episode_index" in episodes.column_names:
        return [int(v) for v in episodes["episode_index"]]
    if isinstance(episodes, list):
        indices = []
        for ep in episodes:
            if isinstance(ep, dict) and "episode_index" in ep:
                indices.append(int(ep["episode_index"]))
        return indices
    return []


def _expand_sparse_episodes(dataset: DebugLeRobotDataset) -> None:
    """Expand metadata so episode_index can be used as a positional index."""
    if dataset.meta.episodes is None:
        dataset.meta.episodes = load_episodes(dataset.meta.root)

    episodes = dataset.meta.episodes
    if not hasattr(episodes, "to_pandas"):
        return

    episodes_df = episodes.to_pandas()
    if episodes_df.empty or "episode_index" not in episodes_df.columns:
        return

    max_idx = int(episodes_df["episode_index"].max())
    expanded = [{} for _ in range(max_idx + 1)]
    for _, row in episodes_df.iterrows():
        ep_idx = int(row["episode_index"])
        expanded[ep_idx] = row.to_dict()

    dataset.meta.episodes = expanded
    dataset.meta.info["total_episodes"] = max_idx + 1
...
'''broken_dataset_root = "datasets/a_fixed"
fixed_dataset_root = "datasets/a_fixed2"
dataset = DebugLeRobotDataset(repo_id="whatevs", root=broken_dataset_root)#, episodes=[0, 3])
assert len(dataset.meta.episodes) == dataset.num_episodes, "Number of episodes in metadata does not match dataset.num_episodes. Adapt this manually in meta > info.json"

dataset.root = fixed_dataset_root
dataset.meta.root = fixed_dataset_root
dataset.meta.info["total_episodes"] = 0
print(f"Old episode indices: {list(dataset.meta.episodes['episode_index'])}")
new_episode_idxs = list(range(len(dataset.meta.episodes)))
print(f"Intended new episode indices: {new_episode_idxs}")
dataset.meta.episodes = dataset.meta.episodes.remove_columns("episode_index").add_column("episode_index", new_episode_idxs)
print(f"New episode indices: {list(dataset.meta.episodes['episode_index'])}")

# Save the updated metadata back to disk
for episode_idx in dataset.meta.episodes["episode_index"]:
    episode_dict = dataset.hf_dataset[episode_idx]
    episode_dict["size"] = None  # these are popped immediately in dataset.save_episode
    episode_dict["task"] = None
    print(episode_dict.keys())
    dataset.save_episode(episode_dict)'''