...
normaliser_checkpoint_path = f"/home/rproesma/Documents/Projects/robot_imitation_glue/outputs/train/b-n200-INSTR1-300k-1enc"
...
# If True, only the first fold is trained. In this mode N_FOLDS controls the
# train/val ratio proxy (val ~= 1/N_FOLDS, train ~= 1 - 1/N_FOLDS).
TRAIN_ONE_FOLD_ONLY = True
EPOCHS = 100
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 1e-6
NUM_WORKERS = 4
...
WANDB_RUN_NAME = f"bb-AUG-{DATASET_ROOT.split('/')[-1]}"
WANDB_ENTITY = "remko-pr-ghent-university"

# Dataset image augmentations, matching clothes-hanger-v3 diffusion config.
IMAGE_TRANSFORMS_ENABLE = True
IMAGE_TRANSFORMS_MAX_NUM = 3
IMAGE_TRANSFORMS_RANDOM_ORDER = False

SAVE_PATH = lambda fold, epoch: (  # for cross validation
    f"outputs/vision_backbones/bb_AUG_{DATASET_ROOT.split('/')[-1]}-fold{fold + 1}-epoch{epoch}.pth"
)
SAVE_PATH_ALL_DATA = lambda epoch: (  # for training on all data (no validation split, no folds)
    f"outputs/vision_backbones/bb_AUG_{DATASET_ROOT.split('/')[-1]}-all-data-epoch{epoch}.pth"
)
FOLDS_CSV_PATH = f"outputs/vision_backbones/bb_AUG_{DATASET_ROOT.split('/')[-1]}-fold_assignments.csv"
...
def build_image_transforms():
    transforms_cfg = ImageTransformsConfig(
        ...
        random_order=IMAGE_TRANSFORMS_RANDOM_ORDER,
        tfs={
    ...
    return ImageTransforms(transforms_cfg) if transforms_cfg.enable else None
    ...
    if TRAIN_ON_ALL_DATA:
        ...
        wandb.init(
            ...
            config={
                ...
                "image_transforms_enable": IMAGE_TRANSFORMS_ENABLE,
                "image_transforms_max_num": IMAGE_TRANSFORMS_MAX_NUM,
                "image_transforms_random_order": IMAGE_TRANSFORMS_RANDOM_ORDER,
    ...
    for fold_idx, val_episode_ids in enumerate(folds):
        ...
        wandb.init(
            ...
            config={
                ...
                "image_transforms_random_order": IMAGE_TRANSFORMS_RANDOM_ORDER,
                "n_folds": N_FOLDS,
                "train_one_fold_only": TRAIN_ONE_FOLD_ONLY,
                "fold_idx": fold_idx + 1,
                "train_steps_per_epoch": train_steps_per_epoch,
                "total_train_steps": total_train_steps,
            },
        ...
        if TRAIN_ONE_FOLD_ONLY:
            logger.info(
                f"TRAIN_ONE_FOLD_ONLY=True: stopping after fold {fold_idx + 1}. "
                f"N_FOLDS={N_FOLDS} was used to set split ratio proxy."
            )
            break

    logger.info(f"total train steps across all folds: {total_train_steps_all_folds}")
    if TRAIN_ONE_FOLD_ONLY:
        logger.info(f"single-fold summary | best val loss: {fold_best_vals[0]:.6f}")
    else:
