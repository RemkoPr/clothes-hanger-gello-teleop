...
SCHUNK_DEFAULT_SPECS = ParallelPositionGripperSpecs(0.0829, 0.005, 150, 30, 0.0575, 0.0055)
...
class SchunkGripperProcess(ParallelPositionGripper):
    ...
    def __init__(
        ...
        self._position = mp.Value("f", 0.0) # in mm
        self._last_target_position = mp.Value("f", 0.0) # in mm
    ...
    def _gripper_process_main(
        ...
        cmd_executing = False
        self.enable_force_monitoring = False
        ...
        while not terminate.is_set():
            ...
            if not cmd_queue.empty() and not cmd_executing:
                ...
                try:
                    ...
                    # For blocking commands, send the result back here (non-blocking
                    # commands already enqueue their own result inside _execute_command).
    ...
    def _execute_command(  # noqa: C901
        ...
        try:
            ...
            if command.cmd_type == "move_pos":
                ...
                if abs(actual_pos - last_target_pos) > 0.1:  # gripper did not reach its previous target
                    ...
                    if last_target_pos > actual_pos and command.params.get("position") > actual_pos:
                        # print(f"new target is same direction: {command.params.get('position')} > {actual_pos}")
                        return True
                    # if new target is same direction: do not execute.
                    if last_target_pos < actual_pos and command.params.get("position") < actual_pos:
                        # print(f"new target is same direction: {command.params.get('position')} < {actual_pos}")
            ...
            elif command.cmd_type == "grip_custom":
                gripper.command_code = eCmdCode.CMD_ACK
                gripper.set_pos = (SCHUNK_DEFAULT_SPECS.max_width - 0.003)*1000
                gripper.command_code = eCmdCode.MOVE_POS
                last_target_position.value = SCHUNK_DEFAULT_SPECS.max_width*1000
                self.enable_force_monitoring = True
                return True

            # in grip mode, the gripper can move at most 20mm/s and 
            # this gets scaled down with lower forces... 
            # so at 50N (min force), the max speed is 5mm/s? not usable for learning..

            # elif command.cmd_type == "grip":
            #     print("gripping")
            #     # stop ongoing move command
            #     gripper.command_code = eCmdCode.CMD_FAST_STOP
            #     print("stopped ongoing command in grip()")
            #     gripper.command_code = eCmdCode.CMD_ACK
            #     print("ACK'ed command in grip()")

            #     # get gripper status
            #     status = gripper.last_statusword
            #     print("--------------------------------")
            #     print(f"gripper status: {status}")
            #     force = command.params.get("force", 100)
            #     gripper.set_force = force
            #     gripper.set_vel = 0
            #     gripper.grp_dir = True
            #     gripper.command_code = eCmdCode.MOVE_FORCE

            #     # After a grip() action the physical jaw position changes, but previously
    
            #     last_target_position.value = SCHUNK_DEFAULT_SPECS.max_width*1000 # internal values for schunk grippers are in mm wrt open pos
    ...
    def move(self, width: float, speed: Optional[float] = None, force: Optional[float] = None) -> AwaitableAction:
        ...
        del force
        # force is not used for move commands!
        # only for grasping.. 
    ...
    #     self._send_command_and_wait_for_result(GripperCommand(cmd_type="grip", params={"blocking": False}))
    ...
    def is_moving(self) -> bool:
        ...
        # print(f"current velocity: {self.get_current_velocity():.5f}")
        return abs(self.get_current_velocity()) > 1e-4

    def grasp_object(self) -> AwaitableAction:
        self._send_command_and_wait_for_result(GripperCommand(cmd_type="grip"))
        return AwaitableAction(not self.is_moving())


if __name__ == "__main__":
    # Example usage
    gripper = SchunkGripperProcess("/dev/ttyUSB1,12")
